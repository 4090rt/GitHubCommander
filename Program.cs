...
class program
{
    ...
    static async Task Main(string[] args)
    {
        ...
        service.AddHttpClient("GithubApiClientDelete", client =>
        {
            client.DefaultRequestHeaders.Accept.ParseAdd("application/vnd.github.v3+json");
            client.DefaultRequestHeaders.UserAgent.ParseAdd("GitHubCommander/1.0");
            client.BaseAddress = new Uri("https://api.github.com/");
            var tolenput = Environment.GetEnvironmentVariable("GITHUB_TOKEN") ?? throw new InvalidOperationException("GITHUB_TOKEN not set");
            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("token", tolenput);
        }).AddTransientHttpErrorPolicy(policy =>
        policy.CircuitBreakerAsync(
            handledEventsAllowedBeforeBreaking: 5,
